package snowtech.com.py.sunshine.app.sync;
...
import snowtech.com.py.sunshine.app.MainActivity;
...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    private ContentProvider mContentResolver;
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        ...
        mContentResolver = provider.getLocalContentProvider();
    ...
    public void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            if (forecastJson.has(OWN_MESSAGE_CODE)) {
                int messageCode = forecastJson.getInt(OWN_MESSAGE_CODE);
        ...
        for (int i = 0; locationId > 0 && i < weatherArray.length(); i++) {
        ...
        if (cVVector.size() > 0) {
            ...
            inserted = mContentResolver.bulkInsert(WeatherContract.WeatherEntry.CONTENT_URI, cArray);

            mContentResolver.delete(WeatherContract.WeatherEntry.CONTENT_URI, where, args);
            notifyWeather();
        }
    ...
    private void notifyWeather() {
        ...
        Boolean notificate = prefs.getBoolean(context.getString(R.string.pref_notification_key),
        ...
        if (System.currentTimeMillis() - lastSync >= DAY_IN_MILLIS) {
            ...
            if (cursor.moveToFirst()) {
                ...
                sendNotification(weatherId, title, contentText);
...
//            mContentResolver.delete(WeatherContract.WeatherEntry.CONTENT_URI, WeatherContract.WeatherEntry.COLUMN_DATE + " <= ?",
//                    new String[] { Long.parseLong(dayTi) })
        } else {
            int idIn = c.getColumnIndex(WeatherContract.LocationEntry._ID);
            id = c.getLong(idIn);
        }
    ...
    static private void setLocationStatus(Context context, @LocationStatus int locationStatus) {
        ...
        ep.putInt(context.getString(R.string.pref_location_status_key), locationStatus);
        ep.commit();
    }

    private void sendNotification(int weatherId, String title, String contentText) {
        Context context = getContext();
